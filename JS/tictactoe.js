...
function placeXOrO(squareNumber) {
    ...
    if (!selectedSquares.some(element => element.includes(squareNumber))) {
        //This vaariable retrieves the html element id that was clicked.
        let select = document.getElementById(squareNumber);
        //This condition checks who's turn it is.
        if (activePlayer === 'X') {
            //If activePlayer is equal to 'X', the x.png is placed in html.
            select.style.backgroundImage = 'url(images/x.png)';
        //Active player ,ay only be 'X' or 'O' so, if not 'X' then 'O'
        }   else {
            //If active player = 'O', the o.png is placed in html
...
//drawWinLine function is called to ddraw line if condition is met.
...
//This function utilizes html canvas to fraw win lines.
function drawWinLine(coordX1, coordY1, coordX2, coordY2) {
    ...
    function animateLineDrawing() {
        ...
        c.stroke();
        //this condiditon checks if we've reached the endpoint
        if (x1 <= x2 && y1 <= y2) {
        ...
        //This condition is necessary for 6, 4, 2 win
        if (x1 <= x2 && y1 >= y2) {
            if (x < x2) {x += 10; }
            if (y > y2) {y -= 10; }
            if (x >= x2 && y <= y2) {cancelAnimationFrame(animationLoop); }
